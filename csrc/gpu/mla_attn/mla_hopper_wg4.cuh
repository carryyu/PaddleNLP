...
template <typename CollectiveMainloop, typename CollectiveEpilogue, typename Ktraits, bool CAUSAL, int SM_COUNT = 132, bool USE_REG_EALLOC=false, bool USE_QK_TWO_STAGE=false>
...
MLAWithKVCacheWG4Kernel(CUTE_GRID_CONSTANT
  ...
  if (warp_group_idx == 0) {
    ...
    const int bid = mainloop_params.batch_ids[blockIdx.x];
    const int tile_id = mainloop_params.tile_ids_per_batch[blockIdx.x];
    const int q_tile_id = mainloop_params.q_tile_ids_per_batch ? mainloop_params.q_tile_ids_per_batch[blockIdx.x] : 0;
